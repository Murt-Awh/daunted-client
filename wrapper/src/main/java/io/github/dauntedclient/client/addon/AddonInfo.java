...
public final class AddonInfo {
	...
	static AddonInfo parse(Path path) throws InvalidAddonException, IOException {
		try (ZipFile zip = new ZipFile(path.toFile())) {
			...
			if (entry == null)
				entry = zip.getEntry("daunted-client-addon.json");
			if (entry == null)
				throw new InvalidAddonException("No addon or daunted-client-addon.json found");
	...
	static AddonInfo parseFromClasspath() throws InvalidAddonException, IOException {
		...
		if (url == null)
			url = wrapper.getResource("daunted-client.addon.json");
		if (url == null)
			throw new InvalidAddonException("No addon or daunted-client-addon.json found in resources");
