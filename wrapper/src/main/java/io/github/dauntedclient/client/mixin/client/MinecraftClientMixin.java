...
import io.github.dauntedclient.client.ui.component.ComponentScreen;
import io.github.dauntedclient.client.ui.screen.*;
...
public abstract class MinecraftClientMixin implements MinecraftClientExtension, MCVer.MinecraftMethodAccessor {
	...
	public void setScreen(Screen screen, CallbackInfo callback) {
		if (((screen == null && world == null) || screen instanceof TitleScreen)
				&& DauntedClientConfig.instance.fancyMainMenu) {
			callback.cancel();
			...
			setScreen(new DauntedClientMainMenu());
			return;
		} else if (screen instanceof DauntedClientMainMenu && !DauntedClientConfig.instance.fancyMainMenu) {
			callback.cancel();
			setScreen(null);
			return;
		}
