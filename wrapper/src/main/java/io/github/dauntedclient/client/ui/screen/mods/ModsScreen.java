...
import io.github.dauntedclient.client.extension.KeyBindingExtension;
import io.github.dauntedclient.client.mod.*;
import io.github.dauntedclient.client.mod.impl.DauntedClientConfig;
import io.github.dauntedclient.client.ui.ScreenAnimation;
...
import io.github.dauntedclient.client.ui.screen.PanoramaBackgroundScreen;
...
public class ModsScreen extends PanoramaBackgroundScreen {
	...
	public void render(int mouseX, int mouseY, float tickDelta) {
		if (client.world == null) {
			if (DauntedClientConfig.instance.fancyMainMenu) {
				background = false;
	...
	public void closeAll() {
		if (client.world == null && DauntedClientConfig.instance.fancyMainMenu) {
	...
	public static class ModsScreenComponent extends BlockComponent {
		...
		public ModsScreenComponent(Mod startingMod) {
			...
			add(new LabelComponent((component, defaultText) -> mod != null ? I18n.translate(mod.getName())
					: I18n.translate("daunted_client.mod.screen.title")).scaled(1.45F),
			...
			if (!singleModMode) {
				add(new ButtonComponent("daunted_client.hud.edit", theme.button(), theme.fg()).onClick((info, button) -> {
					if (button == 0) {
			...
			search = new TextFieldComponent(0, false).autoFlush().onUpdate((ignored) -> {
				...
				scroll.load();
				return true;
			}).withPlaceholder("daunted_client.mod.screen.search").withIcon("search");
		...
		public boolean keyPressed(ComponentRenderInfo info, int keyCode, char character) {
			...
			if (!result) {
				if (keyCode == DauntedClientConfig.instance.modsKey.getCode()
						&& KeyBindingExtension.from(DauntedClientConfig.instance.modsKey).areModsPressed()) {
					mc.setScreen(null);
					return true;
