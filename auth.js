...
class YggdrasilAuthService extends AuthService {
  ...
  authenticateUsername(username) {
    return this.authenticate(new YggdrasilAuthKey(username));
  }

  authenticate(key) {
    // Simulate a successful authentication without making an online request.
    return new Promise((resolve) => {
      const account = new Account("mojang", key.username, "", "offlineAccessToken", "clientToken");
  ...
  validate(_accessToken) {
    // Offline mode doesn't require validation.
    return Promise.resolve(true);
  }

  refresh(account) {
    // Offline mode doesn't require token refresh.
    return Promise.resolve(true);
  }
}
